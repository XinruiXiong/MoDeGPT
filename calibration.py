...
#                 x_in = hidden_states[l].detach().to(torch.float32).mean(dim=1).view(-1)
#                 x_out = hidden_states[l + 1].detach().to(torch.float32).mean(dim=1).view(-1)
#                 if x_in.norm() == 0 or x_out.norm() == 0:
#                     bi_scores[l] += 1.0
#                 else:
#                     cos_sim = cosine_similarity(x_in.unsqueeze(0), x_out.unsqueeze(0), dim=1).item()
